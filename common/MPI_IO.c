...
void init_matrix(	MPI_File f,					//	fichier MPI	//
					MPI_Datatype type, 			// 	type de données a ecrire //
					size_t n)					//	taille de la matrice	//
{
	// taille d'un element
	int size_of_elem;
	...
	// eciture d'un tableau vide
	float *array = calloc(size_of_elem, n * n);
	MPI_File_write_at(f, 0, array, (n * n), type, MPI_STATUS_IGNORE);

	// liberation du tableau vide
	free(array);
}

void set_case(	MPI_File f,					//	fichier MPI	// 
				void *array,				//	tableau dans le quel est alloué l'espace necessaire	//
				MPI_Datatype type, 			// 	type de données a lire	//
				size_t n,					//	taille de la matrice	//
				size_t i, 					//	coordonnées en i de la case	//
...
{	
	...
	// ecriture de la case
	MPI_File_write_at(f, ((i * n) + j) * size_of_elem, array, 1, type, MPI_STATUS_IGNORE);
}

void set_line(MPI_File f,					//	fichier MPI	// 
				void *array,				//	tableau dans le quel est alloué l'espace necessaire	//
				MPI_Datatype type, 			// 	type de données a lire	//
				size_t n,					//	taille de la matrice	//
				size_t i)					//	coordonnées en i de la ligne //
{	
	...
	MPI_File_write_at(f, (i*n) * size_of_elem, array, n, type, MPI_STATUS_IGNORE);
}

void set_column(MPI_File f,					//	fichier MPI	// 
				void *array,				//	tableau dans le quel est alloué l'espace necessaire	//
				MPI_Datatype type, 			// 	type de données a lire	//
				size_t n,					//	taille de la matrice	//
				size_t j)					//	coordonnées en j de la colonne	//
{	
	...
	for(size_t y = 0; y < n; y++){
		MPI_File_write_at(f, ((y * n) + j) * size_of_elem, array + (cpt*size_of_elem), 1, type, MPI_STATUS_IGNORE);
		cpt++;
	}
}


void set_cross(	MPI_File f,					//	fichier MPI	// 
				void *array,				//	tableau dans le quel est alloué l'espace necessaire	//
				MPI_Datatype type, 			// 	type de données a lire	//
				size_t n,					//	taille de la matrice	//
				size_t i, 					//	coordonnées en i de la case d'intersection 	//
...
{	
	...
	// ecriture de la ligne
	MPI_File_write_at(f, (i*n) * size_of_elem, array + (cpt*size_of_elem), n, type, MPI_STATUS_IGNORE);
	cpt += n;
}


void set_block(	MPI_File f,					//	fichier MPI	// 
				void *array,				//	tableau dans le quel est alloué l'espace necessaire	//
				MPI_Datatype type, 			// 	type de données a lire	//
				size_t n,					//	taille de la matrice	//
				size_t ia, 					//	coordonnées en i de la 1ere case //
...
{	
	...
	// ecriture du block
	for(size_t y = ia, cpt = 0; y < ib; y++){
		MPI_File_write_at(f, ((y * n) + ja) * size_of_elem, array + (cpt*size_of_elem), (jb - ja), type, MPI_STATUS_IGNORE);
		cpt += (jb - ja);
	}
}