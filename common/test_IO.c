...
*/
	...
	if(rang == 0){
		...
		get_cross(f, cross, MPI_FLOAT, n, 98, 98);

		for(size_t i = 0; i < n; i++){
			if(cross[i] != matrix[i*n+98]){
				fprintf(stderr, "Erreur case n°%zu | matrix[%zu] = %f et cross[%zu] = %f\n", i, i*n+98, matrix[i*n+98], i, cross[i]);	
			}
		}
		for(size_t i = 0; i < n; i++){
			if(cross[n+i] != matrix[98*n+i]){
				fprintf(stderr, "Erreur case n°%zu | matrix[%zu] = %f et cross[%zu] = %f\n", i, 98*n+i, matrix[98*n+i], i+n, cross[i+n]);	
			}			
		}
	}
/*
